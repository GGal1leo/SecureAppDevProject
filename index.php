...
?>
        ...
        try {
            ...
            if (!in_array('secureappdev', $existingDatabases)) {
				...
				$hashedAdminPass = hashPassword($adminPass);
				
				$makeAdmin = "INSERT INTO `sapusers` (`user_uid`, `user_pwd`, `user_admin`) VALUES ('admin', ?, '1')";
				$stmt = $conn->prepare($makeAdmin);
				$stmt->execute([$hashedAdminPass]);
				echo "Admin Added (Username = admin, Password = AdminPass1!)<br>";
				
				// Hash the user password
				$userPass = "Password1!";
				$hashedUserPass = hashPassword($userPass);
				
				$makeUser = "INSERT INTO `sapusers` (`user_uid`, `user_pwd`, `user_admin`) VALUES ('user1', ?, '0')";
				$stmt = $conn->prepare($makeUser);
				$stmt->execute([$hashedUserPass]);
